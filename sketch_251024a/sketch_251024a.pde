...
// ==========================================================
// FUNÇÃO DO TECLADO (APENAS A BASE)
// ==========================================================
void desenhaTeclado() {
  pushMatrix();
  
  // --- Cores ---
  color tecladoBaseCor = #4A4A4A; // Um cinza escuro para a base do teclado, como na imagem
  color tecladoContorno = #2A2A2A; // Contorno mais escuro
  
  // --- 1. A parte superior arredondada (Arco) ---
  // Esta parte vai se conectar diretamente com a parte de baixo do corpo amarelo.
  // O corpo amarelo está em (0, 100) com altura de 260.
  // Sua borda inferior está em y = 100 + (260/2) = 100 + 130 = 230.
  // Então, o arco precisa começar em y = 230.
  
  float arcY = 230;            // Ponto onde o arco "toca" a base do corpo
  float arcWidth = 330;        // Largura do arco (similar à largura do corpo)
  float arcHeight = 80;        // Profundidade da curva do arco
  float arcOffset = -15;       // Deslocamento para baixo para fazer o encaixe
  
  fill(tecladoBaseCor);
  stroke(tecladoContorno);
  strokeWeight(3);
  
  // Desenha um arco (parte de uma elipse)
  // x, y, width, height, start, stop
  // O (x,y) de um arco é o centro DA ELIPSE COMPLETA.
  // Ajustamos o Y para que a parte de baixo do arco esteja em 'arcY + arcOffset'.
  arc(0, arcY + arcOffset + (arcHeight / 2), arcWidth, arcHeight, PI, TWO_PI);
  
  // --- 2. A parte inferior reta do teclado (Retângulo) ---
  // Conecta-se à parte de baixo do arco e se estende para baixo.
  
  float rectY = arcY + arcOffset + arcHeight / 2 + 70; // Centralizado abaixo do arco
  float rectWidth = 400; // Largura total da base do teclado
  float rectHeight = 160; // Altura da parte reta
  float rectCornerRadius = 15; // Arredondamento dos cantos inferiores
  
  // Desenha o retângulo principal da base
  rect(0, rectY, rectWidth, rectHeight, 0, 0, rectCornerRadius, rectCornerRadius);
